...
class GameScene: SKScene, SKPhysicsContactDelegate  {
    ...
    let itemCategory: UInt32 = 1 << 4 //追加
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            itemScoreLabelNode.text = "CoinScore:\(itemScore)"
                ...
                // 再生データの作成
                let itemGet: SKAction = SKAction.playSoundFileNamed("Points.wav", waitForCompletion: true)
                // 再生アクション.
                self.runAction(itemGet)
                
                print(itemGet)
                contact.bodyA.node?.removeFromParent()//接触したアイテムのみ削除
    ...
    func setupGround() {
        ...
        let moveGround = SKAction.moveByX(-groundTexture.size().width , y: 0, duration: 80.0)
    ...
    func setupItem() {
        ...
        let groundT = SKTexture(imageNamed: "ground")
        let min = UInt32(groundT.size().height + itemTexture.size().height)
        let max = UInt32((self.frame.height - (itemTexture.size().height * 3)))
        //移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + itemTexture.size().width * 6)
        //画面外まで移動するアクションを作成
        let moveItem = SKAction.moveByX(-movingDistance, y: 0, duration: 5.0)
        ...
        //let createItemAnimation = SKAction.runBlock({ updateメソッドで代行
        let item = SKNode()
    ...
    override func update(currentTime: NSTimeInterval) {
        scoreLabelNode.text = "Score:\(score + itemScore)"
        if (time == nil) {
            time = currentTime
        }
